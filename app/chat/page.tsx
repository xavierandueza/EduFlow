...
export default function Home() {
  ...
  const [lastChatAction, setLastChatAction] =
    useState<ChatAction>("unknownResponse");
  const [relevantChatMessage, setRelevantChatMessage] = useState<string>("");
  const [relevantMessagesStartIndex, setRelevantMessagesStartIndex] =
  ...
  const [studentSkill, setStudentSkill] = useState<FirestoreStudentSkill>({
    id: "",
    studentID: "",
    email: "",
    firstName: "",
  ...
  const fetchStudentSkill = async () => {
    ...
    try {
      console.log("Fetching the student skill");
      const response = await fetch("/api/getStudentSkillFirestore", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ studentSkillId }),
      });
  ...
  const fetchCurrentChatAction = async (
    relevantChatMessage,
    studentResponse,
    lastAction,
  ) => {
  ...
  const checkDependencies = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("/api/checkDependenciesFirestore", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ studentSkill }),
      });
      const data = await response.json();
  ...
  }, [studentSkill]); // Update dependencies

  const debouncedCheckDependencies = useCallback(
    debounce(checkDependencies, 500),
    [checkDependencies],
  );
  ...
  const handleSend = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (messages.slice(tempRelevantMessagesStartIndex).length <= 1) {
      ...
      if (
        ...
        relevantChatMessage = messages.slice(tempRelevantMessagesStartIndex)[1]
          .content;
      } else if (
        ...
        messages.slice(tempRelevantMessagesStartIndex)[
          3 + onQuestionLoopCounter * 2 + onFeedbackLoopCounter * 2
        ].content;
      }
      tempChatAction = await fetchCurrentChatAction(
        relevantChatMessage,
        textInput,
        lastChatAction,
      );
          ...
          <div className="chatbot-header pb-6">
            <div className="flex justify-between">
              <div className="flex items-center gap-2">
                <h1
                  className="chatbot-text-primary text-xl md:text-2xl font-medium"
            ...
            <p className="chatbot-text-secondary-inverse text-sm md:text-base mt-2 md:mt-4">
              When you are ready, say that you&apos;re ready, and you will be
              asked a question. Provide an answer, and you will receive some
              feedback on your answer. After you&apos;ve read your feedback, say
              you&apos;re ready for the next question to go again and increase
              your mastery!
            </p>
          </div>

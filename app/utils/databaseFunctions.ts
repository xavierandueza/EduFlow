...
const localAstraDb = new AstraDB(
...
async function getStudentSkillFromDBAll(
  email: string,
  astraDb: AstraDB = localAstraDb,
) {
  try {
...
async function getTeacherFromDB(
  email: string,
  astraDb: AstraDB = localAstraDb,
) {
  try {
...
async function getSchoolClassFromDB(
...
async function getSchoolClassFromDBAll(
  schoolClassNameList: string[],
  astraDb: AstraDB = localAstraDb,
) {
  try {
...
function aggregateSkillsData(skills: StudentSkill[]): SkillAggregate[] {
  try {
    ...
    const uniqueSkills = Array.from(
      new Set(skills.map((skill) => skill.skill)),
    );
    ...
    const studentCount = Array.from(
      new Set(skills.map((skill) => skill.email_address)),
    ).length;
    ...
    for (const skill of uniqueSkills) {
      ...
      const skillStudentSkills = skills.filter(
        (studentSkill) => studentSkill.skill === skill,
      );
...
async function getSkillsAggregateForClassFromDB(
  _id: string,
  astraDb: AstraDB = localAstraDb,
) {
  try {
    ...
    const studentClassCollection =
      await astraDb.collection("school_classes_vec");
    const studentClass = (await studentClassCollection.findOne({
      _id: _id,
    })) as SchoolClass;
    ...
    const cursor = await collection.find({
      school_class_name: studentClass.school_class_name,
    });
...
async function aggregateStudentsData(skills: StudentSkill[]) {
  try {
    ...
    for (const email_address of uniqueStudents) {
      // get the list of studentSkills for this skill
      const studentStudentSkills = skills.filter(
        (studentSkill) => studentSkill.email_address === email_address,
      );
      // loop over the filtered list
      let masteryScoreTotal = 0;
      let retentionScoreTotal = 0;
...
async function getStudentAggregatesForClassFromDB(
  _id: string,
  astraDb: AstraDB = localAstraDb,
) {
  try {
...
async function updateNeedToReviseFlag(
  email: string,
  skill: string,
  ...
  try {
    if (needToRevise) {
      // we have just finished our revision, so halve the decay value
      decayValue = decayValue / 2.0;
    }
    // still missing the ability to update the
    const collection = await astraDb.collection("student_skills_vec");
    const dbResponse = await collection.updateOne(
      { email_address: email, skill: skill },
      { $set: { need_to_revise: !needToRevise, decay_value: decayValue } },
    );
...
async function updateStudentSkillScores(
  email: string,
  skill: string,
  masteryScore: number,
  ...
  try {
    ...
    const newMetricScores = calculateNewMetricScores(
      email,
      skill,
      masteryScore,
      retentionScore,
...
function calculateMetricScoreDelta(
  masteryScore: number,
  retentionScore: number,
  answerGrade: number,
  needToRevise: boolean,
  ...
  if (needToRevise) {
    // calculate the delta from the retentionScore, not the mastery score
...
function calculateNewMetricScores(
  ...
  const metricScoreDelta = calculateMetricScoreDelta(
    masteryScore,
    retentionScore,
    answerGrade,
    needToRevise,
  ...
  if (!needToRevise) {
    ...
    if (updatedRetentionScore > masteryScore) {
      ...
      const updateNeedToRevise = updateNeedToReviseFlag(
        email,
        skill,
        needToRevise,
        decayValue,

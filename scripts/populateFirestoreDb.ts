...
async function main() {
  ...
  for (const schoolClass of schoolClasses) {
    ...
    const studentsQuery = query(
      ...
      where("schoolClassesShort", "array-contains", schoolClass.name),
    );
  ...
  for (const subject of subjects) {
    ...
    const schoolClassesQuery = query(
  ...
  const biologySubjectsQuery = query(
  ...
  const biologySchoolClassesQuery = query(
    collection(db, "schoolClasses"),
  ...
  const biologySkillsQuery = query(
    collection(biologySubjectsSnapshot.docs[0].ref, "skills"),
  );
  ...
  for (const schoolClassesDoc of biologySchoolClassesSnapshot.docs) {
    for (const skillsDoc of biologySkillsSnapshot.docs) {
      ...
      const schoolClassSkillsQuery = query(
        collection(db, "schoolClassSkills"),
        where("schoolClassID", "==", schoolClassesDoc.id),
        where("skillID", "==", skillsDoc.id),
      );
  ...
  const biologyStudentQuery = query(
    collection(db, "students"),
    where("schoolClassesShort", "array-contains", "Biology-1"),
  );
  ...
  const biologySchoolClassSkillsQuery = query(
    collection(db, "schoolClassSkills"),
    where("schoolClass", "==", "Biology-1"),
  );
  const biologySchoolClassSkillsQuerySnapshot = await getDocs(
    biologySchoolClassSkillsQuery,
  );
  ...
  for (const studentDoc of biologyStudentQuerySnapshot.docs) {
    for (const schoolClassSkillsDoc of biologySchoolClassSkillsQuerySnapshot.docs) {
      ...
      const studentSkillsQuery = query(
        studentSkillsCollectionRef,
        where("email", "==", studentDoc.data().email),
        where("skill", "==", schoolClassSkillsDoc.data().skill),
      );
